...
//UART configuration
    ...
    P1SEL = TX + RX;
    P1SEL2 = TX + RX;
    //Parity enabled (UCPEN), Even parity (UCPAR), UART mode (UCMODE_0)
    //LSB first (~UCMSB), 8-bit (~UC7BIT), One stop bit (~UCSPB), Asynchronous (~UCSYNC)
...
__interrupt void UART_interrupt(void){
    if(tx_count<=7)	UCA0TXBUF = send[ tx_count++ ];
    else if(tx_count==8) {UCA0TXBUF = '\r';tx_count++;}
    else{IE2 &= ~UCA0TXIE;tx_count=0;}
}
