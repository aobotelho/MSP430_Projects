...
//Variable used on the TX interruption to count that defines which byte of the array is going to be sent
int tx_count = 0;

//Variable used to store the string that we are going to send on TX
char send[7];

//Variable used to define how many bits are going to be sent on the TX
unsigned int limit=6;

//Variable to control witch input we are sampling on ADC
int axis=1;
...
__interrupt void Timer_A0_interrupt(void){
	...
	for(count=7;count>0;count--){
		if(send[count-2]== ' '||send[count-2]=='.') limit--;
	}
	send[limit+1] = '\r';
	send[limit+2] = 0x0B;
...
__interrupt void UART_interrupt(void){
    if(tx_count<=(limit+2))	UCA0TXBUF = send[ tx_count++ ];
    if(tx_count==(limit+3)){IE2 &= ~UCA0TXIE;tx_count=0;limit=6;}
