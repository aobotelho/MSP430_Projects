...
//***************************************
unsigned char send[8*number_of_conversions+2];
int i;
char teste[10];
unsigned int tx_count=0;
unsigned char axis_names[number_of_conversions] = {'x','y','z','t'};
...
int main(void)
{
  ...
  P1DIR |= TX + LED;
  P1DIR &= ~(RX+Xaxis+Yaxis+Zaxis+Temp);
  P1OUT = LED;
...
//    		  | ISSH		// Set if sample-input is inverted
      ...
      ADC10AE0 |= Xaxis+Yaxis+Zaxis+Temp;
...
//UART configuration
    	...
    	//
    	pos=1;
    	for(counter=0 ; counter<number_of_conversions ; counter++){
    		send[pos] = axis_names[counter];
    		send[pos+1] = ':';
    		send[pos+2] = ' ';
    		pos=pos+8;
    	}
    	...
    	//
    	pos=8;
    	for(counter=0 ; counter<(number_of_conversions-1) ; counter++){
    		send[pos] = ';';
    		pos=pos+8;
    	}


    	send[8*number_of_conversions] = '\r';
    	send[8*number_of_conversions+1] = 0x0B;
...
{	unsigned int pos=(8*number_of_conversions-4);
	unsigned int j;
...
__interrupt void UART_interrupt(void){
    if(tx_count<=(8*number_of_conversions+1))	UCA0TXBUF = send[ tx_count++ ];
    else{
    	IE2 &= ~UCA0TXIE;
    	tx_count=0;
